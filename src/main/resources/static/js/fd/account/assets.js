function show_assets_center(){
    $.ajax({
        type: 'GET',
        url: '/fd/account/assets/get_assets',
        success: function (data) {
            ...
            for (let account_id in json_data) {
                ...
                strs += '<td>' + accountName + '</td>'
                    ...
                    + '<td>' + remark + '</td>'
                    + '<td>'
                    + '<button id="edit_' + id + '" onclick="edit_account_assets(' + id + ')" class="edit_button">edit</button>'
...
function show_account_assets_add_label(year) {
    let str = '';
    str += '<td><input id="account_name_' + year + '" class="input_account_name" type="text"></td>'
        ...
        + '<td>'
        + '<button id="clear_' + year + '" class="button_account_clear" onclick="clear_account_assets_label(' + year + ')">clear</button>'
        + '<button id="add_' + year + '" class="button_account_add" onclick="save_account_assets(' + year + ')">add</button>'
        + '<button id="close_' + year + '" class="button_account_close" onclick="close_add_assets_label(' + year + ')">X</button>'
        + '</td>';
...
function close_add_assets_label(year) {
...
function clear_account_assets_label(year) {
...
function save_account_assets(year) {
...
function edit_account_assets(account_id,is_all=false) {
    let str = '';
    if (is_all){
        str += '<input type="hidden" id="' + account_id + '" value="' + account_id + '">'
            ...
            + '<td>'
            + '<button id="edit_' + account_id + '_add" class="edit_button_account_add" onclick="edit_save_assets(' + account_id + ')">save</button>'
            + '<button id="edit_' + account_id + '_clear" class="edit_button_account_clear" onclick="close_edit_assets(' + account_id + ')">x</button>'
            + '</td>';
    }else{
        str += '<input type="hidden" id="' + account_id + '" value="' + account_id + '">'
            + '<td><input id="edit_' + account_id + '_account_name" class="edit_input_account_name" type="text"></td>'
    ...
    $("#show_edit_assets_box_" + account_id).html(str);

    let edit_box = document.getElementById('show_edit_assets_box_' + account_id);
    edit_box.style.display = "table-row";

    $.ajax({

        type: 'GET',
        url: '/fd/account/assets/edit/' + account_id,
        async: false,
        success: function (data) {
            console.log(data);
...
function edit_save_assets(account_id) {
    let form_data = new FormData();
    ...
    $.ajax({
        type: 'POST',
        url: '/fd/account/assets/edit/save/' + account_id,
        data: form_data,
        dataType: 'json',
        processData: false,
        contentType: false,
...
function close_edit_assets(account_id) {
    let edit_box = document.getElementById('show_edit_assets_box_' + account_id);
...
function delete_account_assets(account_id) {
    console.log(account_id);
    if (account_id !== null) {
        $.ajax({
            type: 'DELETE',
            url: '/fd/account/assets/delete/' + account_id,
            async: false,
            success: function (data) {
                alert(data);
            },
