...
public class RDWorkListController {
    ...
    public ResponseEntity<List<String>> getUserNameList() {
        ...
        List<UserRole> userRoleList = userRoleServiceImpl.findUserRoleByRolesId(RDManagerId);
        userRoleList.addAll(userRoleServiceImpl.findUserRoleByRolesId(RDUserId));
...
//        }
    ...
    public ResponseEntity<List<WorkListDao>> getAllWorkList() {
        Roles userRole = userRoleServiceImpl.findUserRoleByUsersId(getUser().getId()).getRoles();
        Roles role = rolesServiceImpl.findRolesByName("RD_MANAGER");
