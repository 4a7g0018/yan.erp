...
@RequestMapping("/rd/member")
@Controller
@Slf4j
public class RDMemberController {
    ...
    @GetMapping("get_rd_users")
    public ResponseEntity<?> getRdUsers(){
        Long RDManagerId = rolesServiceImpl.findRolesByName("RD_MANAGER").getId();
        Long RDUserId = rolesServiceImpl.findRolesByName("RD_USER").getId();

        List<UserRole> RDUsers=userRoleServiceImpl.findUserRoleByRolesId(RDManagerId);
        RDUsers.addAll(userRoleServiceImpl.findUserRoleByRolesId(RDUserId));

        return ResponseEntity.ok(RDUsers);
    }
